...
- **Blog**: `partials/cta.html` wraps the existing CTA box in `x-data='contactForm({{ t.contact_success|tojson }}, {{ t.contact_error|tojson }})'` (**single-quoted** - `|tojson` always emits double-quoted JS string literals, which terminate a double-quoted HTML attribute at the first embedded quote and truncate the Alpine expression mid-call) and includes `partials/contact_modal.html`, a Bootstrap-styled equivalent of the landing modal (`.modal`-like card + `.form-control`s, visibility driven by Alpine's `x-show`/`x-cloak` rather than Bootstrap's own JS modal component, so the two frameworks' JS never have to interact). Its copy comes from `UI_STRINGS` (`contact_modal_title`/`contact_name`/`contact_email`/`contact_message`/`contact_send`/`contact_sending`/`contact_success`/`contact_error`) like the rest of the blog chrome. Its panel background uses `.bg-body-tertiary`, not `.bg-body` - in dark mode `--bs-body-bg` and `.btn-dark`'s color land on nearly the same near-black, and a `.btn-dark` submit button on a `.bg-body` panel becomes almost invisible.
...
- The Turnstile sitekey (`0x4AAAAAAEM7muzBPUFiEYFd`) is hardcoded in both `content/pages/landing*.html` and `contact_modal.html` - update both if it ever rotates.
