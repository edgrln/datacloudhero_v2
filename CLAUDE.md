...
- **Articles**: `content/blog/fr/`, `content/blog/de/`, `content/blog/es/` (parallel to the flat English ones in `content/blog/`) - same `Slug:` as the English original, `Lang: fr`/`de`/`es`. URLs come from `ARTICLE_LANG_URL`/`ARTICLE_LANG_SAVE_AS` = `{lang}/blog/{slug}/` (set in `pelicanconf.py`, right after the English `ARTICLE_URL`/`ARTICLE_SAVE_AS`) - unrelated to the source directory name, which is purely organizational. **An article with `{attach}`ed images** (e.g. `content/blog/2026-09-14/`) needs its own nested `fr/`/`de`/`es/` subfolder *inside* that article's own folder (not at `content/blog/fr/`), each with its own copy of the image files under the same unsuffixed names - Pelican's `{attach}` only resolves a static file that lives in the same directory as the article referencing it, and only ever attaches one file to one output location, so translations sharing an image file by filename (even across different directories) trigger a Pelican warning ("output location already overridden") and silently wrong image URLs on the 2nd+ article. This also means a translated image usually needs re-authoring (not just relabeling the article) if the image itself has embedded text - see `agent-memory-diagram.png` for the pattern (generated per-language from a small script, not hand-drawn).
